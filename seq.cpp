...
void calculate_aij(Grid& result, double eps) {
    ...
    for (size_t i = 0; i <= result.global_size_x; ++i) {
        for (size_t j = 0; j <= result.global_size_y; ++j) {
...
SolverStats solve_poisson(double a1, double a2, double b1, double b2,
    ...
    Grid r = create_grid(a1, a2, b1, b2, m, n);
...
int main(int argc, char **argv) {
    try {
        if (argc != 4) {
            cerr << "Usage: " << argv[0] << " m n delta_w_stop" << endl;
            return 1;
        }
        ...
        ofstream time_file("serial_times.tsv", ios::app);
        if (time_file) {
            time_file << "M:" << M << " | "
                     << "N:" << N << " | " 
